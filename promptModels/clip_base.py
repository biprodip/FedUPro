...
class CustomCLIP_FC(nn.Module): 
    def __init__(self,classnames, clip_model,n_ctx_num=16,domain_number=6):
        ...
        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype
        # self.cquery = nn.Linear(512,domain_number,bias=False) #domain number is fixed [defaults to client id]
        # self.cquery.half()
...
class CustomCLIP_ad(nn.Module): #adapter
    def __init__(self,classnames, clip_model, n_ctx_num=0,CSC=False):
        super().__init__()
        self.prompt_learner = PromptLearner_client(n_ctx_num, classnames, clip_model) #CSC = True hardcoded
        # self.prompt_learner = PromptLearner_client(n_ctx_num, classnames, clip_model,CSC=CSC) #was in the original code
        self.tokenized_prompts = self.prompt_learner.tokenized_prompts
        self.text_encoder = TextEncoder(clip_model) 
        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype
